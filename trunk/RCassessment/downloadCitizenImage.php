...
// 
// downloadCitizenImage.php - Go to the citizen_images folder and download the relevant images.
//************************************
...
$zip = new ZipArchive;
$zip->open('citizenImageFiles.zip', ZipArchive::OVERWRITE);
while($report = mysql_fetch_array($result)){
	if($report['image_bool']){
		$zip->addFile('./citizen_images/'.$report['image_file_name']);
	}
}
$zip->close();
...
header('Content-disposition: attachment; filename=citizenImageFiles.zip');
readfile('citizenImageFiles.zip');
