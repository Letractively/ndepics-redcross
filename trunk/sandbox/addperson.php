...
if(!$form_filled) { //if the form has not yet been filled out
?>
 <br /><br />			 
<form name='addperson' method='post' action='addperson.php' align ='left'>
    ...
    <br />
    <input type=hidden name='form_filled' value='1'> <!-- Mark that form has been filled out -->
    <input type=hidden name='form_valid' value='0'>  <!-- but has not yet been validated -->
...
<?
} else { //end if first run, else if this is a re-direct to self
$errCount=0; //variable to count errors
//validate each of the inputs, see functions.php for more info
...
if(!$messages[0]) { //if there are no error messages
	$form_valid = 1; //mark the form as valid
}
$messages=array(); //reset the error message array
if($form_valid == 1) { //if the form is marked as valid, forward to addperson2.php
...
} else { //form is not valid, forward to addperson.php (self) for validation
  print "<form name='verifyperson' method='post' action='./addperson.php' align='left'>";
  print "<p align='center'><b>Please make requested corrections and click Continue</b></p>";
}
//table for formatting information printout and correction fields
print "<table>";
//Salutation
validator("Salutation",$salutation,"string","2","10","0");
...
if($errCount > 0) { //if there were errors, mark the form as not validated
  ...
  print "</form>";
} else { //form has been validated, mark as such
...
} //end else form has been submitted (redirection)
