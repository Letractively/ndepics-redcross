...
//	$fs_date = substr($rowfs['date_completed'],5,2) . "/" . substr($rowfs['date_completed'],8,2) . "/" . substr($rowfs['date_completed'],0,4);
	...
	$resultsi = mysql_query($querysi) or die ("Query Failed...could not retreive shelter_info information");
	...
	else print "Email: Nothing was entered <br />";
	if($row['website'] != "")
		print "Website: <a href='http://".$row['website']."'>".$row['website']."</a><br /><br />";
	else print "Website: Nothing was entered <br />";
	if($row['additional_info'] != "")
		print "Additional Information: ".$row['additional_info']."<br />";
	else print "Additional Information: Nothing was entered <br />";
	print "<br />";
	if($row['association'] != NULL)
		print "Associated with: ".$row['association']."<br />";
	else print "Associated with: Nothing was entered <br />";
	if($row['nss_id'] != "")
		print "NSS ID/Code: ".$row['nss_id']."<br /><br />";
	else print "NSS ID/Code: Nothing was entered <br />";
...
//	print "Updated by ".$row['updated_by']." on ".$display_date."<br /><br />";
	...
	if($rowsi['organization_id'] != ""){
		...
		//SoU File
	if($row['statement_understanding'] != NULL){
		print "Statement of Understanding File: Uploaded <br />";
		...
		{
			echo"<br /><a href=\"SOUs/".$row['statement_understanding']."\">View Statment of Understanding</a> <br />";
		}
	}
	else
		print "No Statement of Understanding<br />";
		// SoU Expiration Date
	if($row['statement_expiration'] != NULL){
		$expirationDateRaw = $row['statement_expiration'];
		$expirationDate = "";
		for($i=5; $i < strlen($expirationDateRaw); $i++){
			$expirationDate = $expirationDate.$expirationDateRaw[$i];
		}
		$expirationDate = $expirationDate."-";
		for($i=0; $i < 4; $i++){
			$expirationDate = $expirationDate.$expirationDateRaw[$i];
		}
		print "SoU Expiration Date: ".$expirationDate."<br />";
		
	}
	else
		print "The Statement of Understanding has no Expiration Date <br />"; 
		
		//FS
	if($row['facility_survey'] != ""){
		print "Facility Survey uploaded<br />";
		if( !( ($_SESSION['access_level_id'] != 1) && ($_SESSION['access_level_id'] != 3) && ($_SESSION['access_level_id'] != 5) && ($_SESSION['access_level_id'] != 7) && ($_SESSION['access_level_id'] != 9)) )
		{
			echo"<br /><a href=\"FSs/".$row['facility_survey']."\">View Facility Survey</a> <br />";
		}
	}
	else
